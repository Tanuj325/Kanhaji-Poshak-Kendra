      ...
      <div className="space-y-5 sm:space-y-6 font-display min-w-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3.5 sm:gap-4 border-b border-slate-200/60 pb-4 sm:pb-5">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-semibold text-slate-700 shadow-2xs hover:bg-slate-50 transition-all disabled:opacity-50 min-h-[40px] sm:min-h-0 shrink-0 cursor-pointer self-start sm:self-auto"
          >
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-3.5 sm:p-4 shadow-xs min-w-0">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3.5 sm:gap-4 min-w-0">
            <div className="flex items-center gap-1.5 bg-slate-100 p-1.5 rounded-xl overflow-x-auto custom-scrollbar max-w-full min-w-0" role="radiogroup" aria-label="Sales date range filter">
              {RANGES.map((r) => (
                <button
                  key={r.id}
                  onClick={() => setRange(r.id)}
                  className={cn(
                    'px-3 py-1.5 text-xs font-bold rounded-lg transition-all focus:outline-none whitespace-nowrap shrink-0 cursor-pointer min-h-[36px] sm:min-h-0',
                    range === r.id
            ...
            {range === 'custom' && (
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3 bg-slate-50 p-2.5 rounded-xl border border-slate-200 min-w-0">
                <div className="flex items-center gap-2 min-w-0">
                  <FiCalendar className="h-4 w-4 text-amber-600 shrink-0" />
                  <span className="text-xs font-semibold text-slate-700 shrink-0">From:</span>
                  <Input
                    type="date"
                    value={startDateStr}
                    onChange={(e) => setStartDateStr(e.target.value)}
                    className="py-1.5 px-2 text-xs bg-white min-h-[38px] sm:min-h-0"
                    aria-label="Start Date"
                  />
                </div>
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-xs font-semibold text-slate-700 shrink-0">To:</span>
                  <Input
                    type="date"
                    value={endDateStr}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-4 min-w-0">
          <div className="rounded-2xl border border-slate-200/80 bg-white p-4 sm:p-5 shadow-xs min-w-0">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                Total Revenue
              </span>
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-amber-500/10 text-amber-600 border border-amber-500/20 shrink-0">
            ...
            <div className="mt-2.5 sm:mt-3 min-w-0">
              {isLoading ? (
                ...
                <p className="font-serif text-xl sm:text-2xl font-bold text-slate-900 truncate">
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white p-4 sm:p-5 shadow-xs min-w-0">
            <div className="flex items-center justify-between">
              ...
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-purple-500/10 text-purple-600 border border-purple-500/20 shrink-0">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-4 sm:p-6 shadow-xs min-w-0 overflow-hidden">
          <div className="flex items-center justify-between mb-4 sm:mb-5 min-w-0">
            <div className="min-w-0">
              <h3 className="font-serif text-base sm:text-lg font-bold text-slate-900 truncate">Revenue Trend</h3>
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-4 sm:p-6 shadow-xs min-w-0 overflow-hidden">
          <div className="flex items-center justify-between mb-4 min-w-0">
            <h3 className="font-serif text-base font-bold text-slate-900 truncate">Sales Period Breakdown</h3>
            <span className="text-xs font-semibold text-slate-500 shrink-0">{salesData.length} records</span>
          </div>
            ...
            <div className="overflow-x-auto custom-scrollbar rounded-xl border border-slate-100">
              <table className="w-full text-left text-xs" aria-label="Sales breakdown details">
                <thead className="bg-slate-50 text-[10px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200/80">
                  <tr>
                    <th scope="col" className="py-3 px-3.5 sm:px-4">Period / Date</th>
                    <th scope="col" className="py-3 px-3.5 sm:px-4 text-right">Completed Orders</th>
                    <th scope="col" className="py-3 px-3.5 sm:px-4 text-right">Total Revenue</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {salesData.map((row, idx) => (
                    <tr key={idx} className="hover:bg-slate-50 transition-colors">
                      <td className="py-3 px-3.5 sm:px-4 font-semibold text-slate-900 whitespace-nowrap">{row.label}</td>
                      <td className="py-3 px-3.5 sm:px-4 text-right font-mono font-medium text-slate-600">{row.orders ?? 0}</td>
                      <td className="py-3 px-3.5 sm:px-4 text-right font-mono font-bold text-amber-700 whitespace-nowrap">
