      ...
      <div className="space-y-3.5 sm:space-y-4 font-display max-w-5xl min-w-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 border-b border-slate-200/60 pb-3 min-w-0">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-[11px] text-slate-500 font-body truncate">
              Interface preferences, system refresh rates, shortcuts, and view environment status
            </p>
          </div>
          <button
            type="button"
            onClick={handleSavePreferences}
            className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-amber-500 hover:bg-amber-600 px-3 py-1.5 text-xs font-semibold text-white shadow-2xs transition-all active:scale-[0.98] min-h-[36px] sm:min-h-0 shrink-0 cursor-pointer self-start sm:self-auto"
          >
        ...
        <div className="rounded-xl border border-slate-200/80 bg-white p-3.5 sm:p-4 shadow-2xs space-y-3 min-w-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5 sm:gap-3 text-xs min-w-0">
            <div className="p-2.5 sm:p-3 rounded-lg bg-slate-50/80 border border-slate-200/70 space-y-0.5 min-w-0">
            ...
            {SHORTCUTS.map((item) => {
              ...
              return (
                <Link
                  key={item.title}
                  to={item.to}
                  className="group relative flex items-center justify-between gap-2.5 p-2.5 rounded-lg border border-slate-200/80 bg-slate-50/60 hover:border-amber-400/80 hover:bg-amber-50/40 transition-all duration-150 min-w-0 cursor-pointer"
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-amber-500/10 text-amber-700 border border-amber-500/20 group-hover:bg-amber-500 group-hover:text-slate-950 group-hover:border-amber-400 transition-all shrink-0 shadow-2xs">
                      <Icon className="h-3.5 w-3.5" />
                    </div>
                    <div className="min-w-0">
                      <h4 className="font-semibold text-xs text-slate-900 group-hover:text-amber-950 transition-colors truncate">
                        {item.title}
                      </h4>
                      <p className="text-[10px] text-slate-500 group-hover:text-slate-600 truncate">
                        {item.subtitle}
                      </p>
                    </div>
                  </div>
