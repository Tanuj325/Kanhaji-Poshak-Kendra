            ...
            <div className="block lg:hidden space-y-3">
              {orders.map((row) => (
                <div
                  key={row.id}
                  onClick={() => navigate(buildPath.adminOrderDetail(row.id))}
                  className="rounded-2xl border border-slate-200/80 bg-white p-3.5 sm:p-4 shadow-2xs hover:border-amber-500/30 transition-all cursor-pointer space-y-3"
                >
                  ...
                  <div className="flex items-center justify-between gap-2 border-b border-slate-100 pb-2.5">
                    ...
                    <span
                      className={cn(
                        'inline-block border px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider shrink-0',
                        STATUS_BADGES[row.orderStatus] || 'bg-slate-100 text-slate-600 border-slate-200'
                      )}
                    >
                      {(row.orderStatus || 'PENDING').replace(/_/g, ' ')}
                  ...
                  {/* Middle Row: Customer Info & Total Price */}
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <p className="text-sm font-mono font-extrabold text-slate-900">
                        {formatPrice(row.totalAmount)}
                      </p>
                    </div>
                  </div>

                  {/* Bottom Row: Payment & Action Control */}
                  <div
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0 ml-auto">
                      <select
                        value={row.orderStatus || ''}
                        onChange={(e) => handleStatusUpdate(row.id, e.target.value)}
                        className="text-[10px] font-bold py-1.5 px-2 rounded-xl border border-slate-200 bg-slate-50 text-slate-700 focus:border-amber-500 focus:outline-none min-h-[36px]"
                      >
                        {ORDER_STATUSES.map((s) => (
                          <option key={s} value={s}>
                            {s.replace(/_/g, ' ')}
                      ...
                      <button
                        type="button"
                        onClick={() => navigate(buildPath.adminOrderDetail(row.id))}
                        className="flex items-center gap-1 rounded-xl bg-amber-500/10 px-2.5 py-1.5 text-xs font-bold text-amber-800 hover:bg-amber-500/20 transition-colors min-h-[36px]"
                      >
