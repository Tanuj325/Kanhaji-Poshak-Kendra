...
function AddressCardSkeleton() {
  return (
    <div className="w-full rounded-[22px] p-4 sm:p-5 border border-amber-900/10 bg-white space-y-3 animate-pulse shadow-2xs overflow-hidden">
...
function AddressEmptyState({ onAdd }) {
  return (
    <div className="w-full rounded-[24px] bg-white border border-amber-900/10 p-6 sm:p-10 text-center space-y-4 shadow-sm my-4 overflow-hidden">
...
// ----------------------------------------------------
// Rebuilt Premium Address Card (Responsive & No Overflow)
// ----------------------------------------------------
function RebuiltAddressCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        'relative w-full max-w-full rounded-[22px] p-4 sm:p-5 transition-all duration-300 font-display overflow-hidden text-left flex flex-col justify-between gap-4 border',
        isSelected && isSelectable
          ? 'border-2 border-[#D4AF37] bg-gradient-to-b from-[#FFFDF9] to-[#FAF2E4] shadow-md ring-2 ring-[#D4AF37]/20'
          : 'border-amber-900/12 bg-gradient-to-b from-white via-[#FCFBF8] to-[#FAF6F0] hover:border-amber-700/40 shadow-[0_4px_24px_rgba(44,40,36,0.05)] hover:shadow-[0_8px_30px_rgba(44,40,36,0.1)]',
    ...
    >
      ...
      <div className="space-y-3 min-w-0 w-full">
        {/* Top Header Row: Icon/Radio, Name & Badges */}
        <div className="flex items-start gap-2.5 min-w-0 w-full">
          ...
          {isSelectable ? (
            ...
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-amber-100/80 border border-amber-300/40 text-amber-900 flex items-center justify-center shrink-0 shadow-2xs mt-0.5">
              <FiMapPin className="w-4 h-4 text-amber-900" />
            </div>
          )}

          <div className="min-w-0 flex-1 space-y-1.5">
            <div className="flex items-center gap-1.5 flex-wrap min-w-0">
              <span className="font-heading font-black text-sm sm:text-base text-amber-950 tracking-tight truncate max-w-full">
                {address.fullName}
              </span>
              ...
              {(isOffice || isHome) && (
                <span
                  className={cn(
                    'inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full border shadow-2xs shrink-0',
                    isOffice
              ...
              {address.defaultAddress && (
                <span className="inline-flex items-center gap-1 text-[10px] font-black text-amber-100 bg-gradient-to-r from-amber-900 via-amber-800 to-amber-950 px-2.5 py-0.5 rounded-full border border-amber-400/40 uppercase tracking-wider shadow-2xs shrink-0">
            ...
            {/* Phone Badge */}
            {address.phoneNumber && (
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-amber-50/80 border border-amber-900/10 text-xs font-bold font-mono text-amber-950 shadow-2xs max-w-full truncate">
                <FiPhone className="h-3 w-3 text-amber-800 shrink-0" />
                <span className="truncate">{address.phoneNumber}</span>
                <button
                  type="button"
                  onClick={copyPhoneNumber}
                  className="p-0.5 text-stone-400 hover:text-amber-900 transition-colors shrink-0"
        ...
        <div className="p-3.5 rounded-2xl bg-white/90 border border-amber-900/10 space-y-1 font-body text-xs sm:text-sm leading-relaxed text-stone-700 shadow-2xs min-w-0 w-full overflow-hidden">
          <p className="font-semibold text-stone-800 break-words">{address.addressLine1}</p>
          {address.addressLine2 && <p className="text-stone-600 break-words">{address.addressLine2}</p>}
          <p className="font-extrabold text-amber-950 text-xs sm:text-sm font-display pt-0.5 break-words">
            {address.city}, {address.state} — {address.postalCode}
          </p>
          {address.country && address.country !== 'India' && (
            <p className="text-[10px] font-extrabold uppercase tracking-widest text-stone-400 pt-0.5 break-words">
              {address.country}
            </p>
          )}
        </div>
      ...
      {/* Action Buttons Bar */}
      <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-amber-900/10 font-display min-w-0 w-full">
        <div className="flex items-center gap-2">
          {onEdit && (
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-extrabold text-amber-950 hover:bg-amber-100/80 bg-white border border-amber-900/15 px-3 py-1.5 rounded-xl transition-all shadow-2xs active:scale-95 min-h-[36px]"
            >
          ...
          {onDelete && (
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-extrabold text-rose-700 hover:bg-rose-100/80 bg-rose-50/80 border border-rose-200/60 px-3 py-1.5 rounded-xl transition-all shadow-2xs active:scale-95 min-h-[36px]"
            >
        ...
        {onSetDefault && !address.defaultAddress && (
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-extrabold text-white bg-gradient-to-r from-amber-800 via-amber-900 to-amber-950 hover:from-amber-900 hover:to-stone-950 px-3.5 py-1.5 rounded-xl transition-all shadow-xs active:scale-95 min-h-[36px] border border-amber-500/20"
          >
...
export default function MobileShippingAddress({
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-[#FAF7F2] font-display flex flex-col justify-between overflow-y-auto lg:hidden">
      ...
      {/* ---------------------------------------------------- */}
      <main className={cn('flex-1 w-full max-w-5xl mx-auto px-3.5 sm:px-6 py-4 md:py-6 space-y-4', mode === 'checkout' ? 'pb-28' : 'pb-16')}>
        {isLoading ? (
          ...
          <>
            ...
            <div
              className="grid grid-cols-1 md:grid-cols-2 gap-3.5 md:gap-4.5 w-full min-w-0"
            ...
            >
              <div className="w-6 h-6 rounded-full bg-amber-900 text-white flex items-center justify-center shadow-2xs">
                <FiPlus className="w-4 h-4 text-amber-100" />
              </div>
      ...
      {/* ---------------------------------------------------- */}
      {/* ADDRESS FORM MODAL / BOTTOM SHEET (z-[80] ABOVE ALL NAV BARS) */}
      {/* ---------------------------------------------------- */}
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[80] flex items-end md:items-center justify-center bg-stone-950/60 backdrop-blur-xs p-0 md:p-4">
            <motion.div
              initial={{ y: '100%', opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              exit={{ y: '100%', opacity: 0 }}
              transition={{ type: 'spring', damping: 25, stiffness: 280 }}
              className="w-full max-w-lg md:max-w-xl bg-white rounded-t-[28px] md:rounded-[28px] shadow-2xl overflow-hidden font-display flex flex-col max-h-[92vh] mb-0 md:mb-auto"
            >
              {/* Form Modal Header */}
              <div className="px-5 py-4 border-b border-amber-900/10 flex items-center justify-between bg-amber-50/40 shrink-0">
              ...
              <form onSubmit={handleFormSubmit} className="flex flex-col flex-1 overflow-hidden font-display">
                <div className="p-5 space-y-[14px] overflow-y-auto flex-1 font-display pb-6">
                ...
                {/* Sticky Submit / Action Bar at Bottom of Modal */}
                <div className="px-5 py-3 border-t border-amber-900/10 flex items-center justify-end gap-2.5 bg-white shrink-0 sticky bottom-0 z-10 shadow-[0_-4px_12px_rgba(0,0,0,0.04)]">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
      ...
      {/* Delete Confirmation Dialog (z-[90] ABOVE ALL) */}
      <ConfirmDialog
