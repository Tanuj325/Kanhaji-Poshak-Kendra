...
export default function PublicLayout() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#FAF7F2] text-amber-950 font-display selection:bg-amber-400/30 selection:text-amber-950">
      ...
      <main className="flex-1 w-full min-w-0 overflow-x-hidden pb-bottom-nav md:pb-0">
        <Suspense fallback={<LoadingSection />}>
          <Outlet key={location.pathname} />
        </Suspense>
      </main>

      {/* Desktop & Tablet Footer (hidden on mobile app experience if needed, or preserved) */}
      <div className="hidden md:block">
        <Footer />
      </div>
