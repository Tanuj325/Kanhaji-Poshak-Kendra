...
export default function MobileDashboard() {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-[#F9F7F4] text-stone-900 font-display antialiased overflow-y-auto lg:hidden pb-28 sm:pb-12">
      {/* ─── 1. TOP HEADER BAR (← Back & Notifications) ─── */}
      <header className="sticky top-0 z-30 h-[56px] w-full bg-[#F9F7F4]/90 backdrop-blur-xl border-b border-stone-200/80 px-4 sm:px-5 flex items-center justify-between">
        <button
          type="button"
          onClick={() => navigate(-1)}
        ...
        <Link
          ...
          className="relative h-8 w-8 rounded-full bg-white text-stone-800 border border-stone-200/80 flex items-center justify-center active:scale-95 transition-transform shadow-2xs shrink-0"
          aria-label="Notifications"
        >
          <FiBell className="w-[18px] h-[18px] text-stone-800" />
          {unreadCount > 0 && (
            <span className="absolute -top-1 -right-1 h-[14px] min-w-[14px] px-1 rounded-full bg-rose-600 text-white text-[10px] font-extrabold flex items-center justify-center shadow-2xs border border-white">
      ...
      {/* ─── 2. PAGE HEADING & GREETING ─── */}
      <div className="w-full px-4 sm:px-5 pt-3 pb-1">
        <h1 className="text-[30px] font-heading font-extrabold text-amber-950 tracking-tight leading-none">
          My Account
        </h1>
        <p className="text-[16px] text-stone-600 font-medium mt-1">
      ...
      <main className="w-full px-4 sm:px-5 pt-2 space-y-5">
        {/* ─── 3. PROFILE CARD REDESIGN (Height 170-190px, Radius 20px, Luxury Cream Gradient) ─── */}
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.25 }}
          className="w-full min-h-[180px] rounded-[20px] bg-gradient-to-br from-[#FFFDF7] via-[#FDF5E6] to-[#F5E6CA] border border-[#E6C687]/60 p-4 shadow-[0_8px_24px_-4px_rgba(201,154,59,0.12)] relative overflow-hidden flex flex-col justify-between gap-3"
        >
          {/* TOP ROW: Avatar (60px), Name (22px), Email (13px), Phone (13px), Edit Button */}
          <div className="flex items-center justify-between gap-3 w-full">
            <div className="flex items-center gap-3.5 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                <h2 className="text-[22px] font-heading font-extrabold text-amber-950 truncate leading-snug">
                  {fullName}
                </h2>
                {email && (
                  <p className="text-[13px] text-stone-600 truncate font-medium flex items-center gap-1 mt-0.5">
                    <FiMail className="w-[14px] h-[14px] text-amber-900 shrink-0" />
                    <span className="truncate">{email}</span>
                  </p>
                )}
                {phone && (
                  <p className="text-[13px] text-stone-600 truncate font-medium flex items-center gap-1 mt-0.5">
                    <FiPhone className="w-[14px] h-[14px] text-amber-900 shrink-0" />
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-full bg-amber-950 hover:bg-amber-900 active:scale-95 text-white text-[13px] font-bold shadow-2xs transition-all flex items-center gap-1 shrink-0"
            >
              <FiEdit3 className="w-[14px] h-[14px]" />
              <span>Edit</span>
            </button>
          </div>

          {/* BOTTOM ROW: Stats bar (Orders, Wishlist, Coupons) */}
          <div className="w-full bg-white/80 backdrop-blur-md rounded-[14px] p-2 border border-amber-300/40 flex items-center justify-around">
            <div
              onClick={() => navigate(ROUTE_PATHS.ORDERS)}
              className="flex flex-col items-center flex-1 border-r border-amber-900/10 last:border-r-0 cursor-pointer active:scale-95 transition-transform"
            >
              <span className="text-[16px] font-extrabold font-mono text-amber-950 leading-none">
                {totalOrders}
              </span>
              <span className="text-[12px] font-semibold text-stone-600 mt-1 uppercase tracking-wider">
                Orders
              </span>
            </div>
        ...
        {/* ─── 4. QUICK ACTION GRID (2 Columns, Height 92px, Rounded 18px, Icon 22px, Title 16px, Subtitle 13px) ─── */}
            ...
            {quickActions.map((action, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="w-full h-[92px] rounded-[18px] bg-white border border-stone-200/80 p-3 shadow-2xs hover:border-amber-400/60 cursor-pointer flex flex-col justify-between transition-all group"
                >
                  <div className="flex items-center justify-between w-full">
                    <Icon className="w-[22px] h-[22px] text-amber-900 stroke-[1.8] group-hover:scale-105 transition-transform shrink-0" />
                  ...
                  <div className="w-full min-w-0">
                    <div className="text-[16px] font-heading font-bold text-stone-900 group-hover:text-amber-950 truncate leading-snug">
                      {action.label}
                    </div>
        ...
        {/* ─── 5. ACCOUNT OPTIONS (Apple Settings Style, Rows 58px Height, Icon 20px, Title 15px) ─── */}
        <section className="w-full space-y-3">
          <h3 className="text-[16px] font-heading font-extrabold text-amber-950 px-0.5">
            Account Options
          </h3>
        ...
        {/* ─── 6. PREMIUM iOS SIGN OUT BUTTON (56px Height, 18px Radius, Red Outline) ─── */}
