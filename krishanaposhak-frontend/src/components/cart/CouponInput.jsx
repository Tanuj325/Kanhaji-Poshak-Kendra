...
import { FiTag, FiCheckCircle, FiTrash2, FiStar, FiGift } from 'react-icons/fi';
...
function CouponInput({ orderAmount = 0, appliedCoupon, onApply, onRemove, className }) {
  ...
  if (appliedCoupon) {
    return (
      <div
        className={`rounded-2xl bg-gradient-to-r from-emerald-50 via-emerald-100/50 to-emerald-50 border border-emerald-300/60 p-4 shadow-sm relative overflow-hidden font-display ${className || ''}`}
      >
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-base font-extrabold text-stone-950 font-mono tracking-wider">
                {appliedCoupon.code}
              </p>
  ...
  return (
    <div
      className={`rounded-2xl bg-white/90 backdrop-blur-md border border-amber-900/10 p-4 sm:p-5 shadow-xs space-y-3 font-display ${className || ''}`}
    >
      <label
        htmlFor="coupon-code-input"
        className="block text-xs font-bold uppercase tracking-wider text-stone-900 flex items-center gap-2"
      >
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-xl border border-amber-900/20 bg-stone-50/50 px-3.5 py-2.5 text-xs font-bold uppercase tracking-wider text-stone-950 focus:border-amber-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-400/20 font-mono transition-all"
        />
        <Button
          variant="primary"
          size="sm"
          ...
          disabled={!code.trim()}
          className="font-bold min-h-[40px] px-4 rounded-xl"
        >
          Apply
        </Button>
      </div>
      ...
      {/* Backend Active Coupon Suggestions List */}
